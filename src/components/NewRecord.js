...
class NewRecord extends Component {
  ...
  onSubmit(event) {
    ...
    console.log(this.state);
    contract.methods.addrecordReport(record_id, address, timestamp, record_code, description).send({ from: accounts[0] })
      .then(result => {
  ...
  onGetDate() {
    ...
    const timestamp = `${year}-${month}-${day} ${hour}:${mins}:${sec}`;
            ...
            <div className="col s6">
              <div className="card reportCard">
                ...
                <div className="card-content">
                  <form onSubmit={this.onSubmit}>
                    <div className="input-field">
                      <input type="text" id="recordId" onChange={(evt) => this.setState({ record_id: evt.target.value })} required />
                      <label htmlFor="recordId">Student ID</label>
                    </div>
                    <div className="input-field">
                    ...
                    <div className="input-field">
                      <input type="text" id="recordCode" onChange={(evt) => this.setState({ record_code: evt.target.value })} required />
                      <label htmlFor="recordCode">Record Name</label>
                    </div>
                    <div className="form-submit center">
                      <button type="submit" className="dropbtn1" style={{ marginTop: "10px" }}>Upload to Blockchain</button>
                    </div>
